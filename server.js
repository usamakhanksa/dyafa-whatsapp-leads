...
app.post('/send-message', async (req, res) => {
  const { message, visitorName = 'Website Visitor', visitorPhone = '' } = req.body;
  if (!message) return res.status(400).json({ success: false, error: 'Message required' });

  if (!clientReady) return res.status(503).json({ success: false, error: 'Scan QR first' });
...
app.get('/qr', (req, res) => {
  ...
  res.send(`<h2 style="text-align:center;color:#25D366">Scan QR</h2><img src="${lastQrDataUrl}" style="max-width:400px">`);
});
